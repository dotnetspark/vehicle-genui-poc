...
describe("resolveResourceUri — integration (actual dist/resource-uri.json)", () => {
  it("returns a URI that matches the format in dist/resource-uri.json", async () => {
  ...
  it("URI from resolveResourceUri() matches the uri field in dist/resource-uri.json", async () => {
    const moduleDir = path.dirname(fileURLToPath(import.meta.url));
